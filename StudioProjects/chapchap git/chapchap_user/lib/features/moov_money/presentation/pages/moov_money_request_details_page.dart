...
class _MoovMoneyRequestDetailsPageState extends State<MoovMoneyRequestDetailsPage> {
  ...
  void _listenToRealtimeUpdates() {
    ...
    _hybridService.startListening(
      ...
      onStatusChanged: (status) {
        ...
        // Si la transaction est complétée, afficher le dialogue de fin
        if (status.toLowerCase() == 'completed' && mounted) {
  ...
  /// Affiche le dialogue de fin de transaction demandant si l'utilisateur veut faire une autre opération
  void _showTransactionCompletedDialog() {
    // Annuler les listeners pour éviter les notifications multiples
    _realtimeSubscription?.cancel();
    _pollingTimer?.cancel();
    _hybridService.dispose();
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            Expanded(
              child: Text(
                type == 'deposit' ? 'Dépôt réussi!' : 'Retrait réussi!',
        ...
        content: Column(
          ...
          children: [
            Text(
              'Votre ${type == 'deposit' ? 'dépôt' : 'retrait'} de $amount FCFA a été effectué avec succès!',
        ...
        actions: [
          TextButton(
            ...
            child: const Text('Non, retour accueil'),
          ),
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context); // Fermer le dialog
              // Retourner à la page Moov Money pour une nouvelle opération
            ...
            child: const Text('Oui, autre opération'),
          ),
        ],
      ),
    );
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Pour les retraits ET dépôts: afficher le bouton de confirmation
                        // (En attendant que Moov configure le callback automatique)
                        Builder(
                          builder: (context) {
                            final type = (_request!['type'] ?? _request!['moov_money_type'] ?? '').toString().toLowerCase();
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Copier le code', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(
  ...
  /// Widget d'attente de confirmation USSD pour les retraits
  /// Le user doit entrer son code PIN sur son téléphone via USSD
  Widget _buildUssdWaitingCard() {
    final amount = _request!['moov_money_amount'] ?? _request!['amount'] ?? 0;
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Vous avez reçu un message USSD.\nEntrez votre code PIN Moov Money pour confirmer le retrait.',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 14,
                    color: Colors.white.withOpacity(0.9),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'La transaction sera automatiquement confirmée après validation de votre PIN',
                    style: TextStyle(
                      fontSize: 12,
                      color: Colors.white.withOpacity(0.9),
                    ),
