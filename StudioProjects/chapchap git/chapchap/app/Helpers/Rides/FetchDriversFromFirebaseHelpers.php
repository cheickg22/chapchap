...
trait FetchDriversFromFirebaseHelpers
{
    ...
    protected function fetchDriversFromFirebase($request_detail,$database)
    {
        ...
        // Skip Moov Money requests - they don't need driver dispatch via Firebase
        if ($request_detail->is_moov_money) {
            Log::channel('activity')->info(['FetchDrivers'=>'Skipping Moov Money request', 'request_id' => $request_detail->id]);
            return null;
        }

        // Verify zoneType exists before accessing its properties
        if (!$request_detail->zoneType) {
            Log::channel('activity')->warning(['FetchDrivers'=>'No zoneType for request', 'request_id' => $request_detail->id]);
            return null;
        }
        ...
        $type_Capacity = $request_detail->zoneType->vehicleType?->capacity ?? 4;
