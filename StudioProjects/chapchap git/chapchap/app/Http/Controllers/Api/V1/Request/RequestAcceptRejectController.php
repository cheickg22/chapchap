...
class RequestAcceptRejectController extends StripeController
{
    ...
    public function respondRequest(AcceptRejectRequest $request)
    {
        ...
        */
        ...
        if ($request->input('is_accept')) {
            ...
            // ✅ Si c'est une requête Moov Money, mettre à jour Firebase
            if ($request_detail->is_moov_money) {
                $this->updateMoovMoneyFirebase($request_detail, 'accepted', $driver);
            }
    ...
    */
    ...
    {
        $driver = auth()->user()->driver;
        if($driver->requestDetail()->where('is_cancelled', false)->where('is_completed', false)->where('is_driver_started',false)->exists()){
    ...
    /**
     * Mettre à jour Firebase pour les requêtes Moov Money
     */
    protected function updateMoovMoneyFirebase($request, $status, $driver)
    {
        try {
            $updateData = [
                'status' => $status,
                ...
                // ✅ Inclure le montant et les infos Moov Money
            ...
            \Log::info('✅ Firebase updated for Moov Money (driver accepted)', [
            ...
            \Log::error('❌ Failed to update Firebase for Moov Money', [
                'request_id' => $request->id,
                'error' => $e->getMessage(),
            ]);
        }
