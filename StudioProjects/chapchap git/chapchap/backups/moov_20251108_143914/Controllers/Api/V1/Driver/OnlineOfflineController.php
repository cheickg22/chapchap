...
class OnlineOfflineController extends BaseController
{
    ...
    */
    public function toggle()
    {
        ...
        // Logs de débogage
        \Log::info('=== ONLINE/OFFLINE DEBUG ===');
        \Log::info('Status avant save: active=' . $driver->active . ', available=' . $driver->available);
        \Log::info('Nouveau status: ' . ($status ? 'true' : 'false'));
        
        $driver->active = $status;
        $driver->available = $status;
        $driver->save();
        
        \Log::info('Status après save: active=' . $driver->active . ', available=' . $driver->available);
        
        // Forcer les attributs pour éviter que le transformer ne les modifie
        $driver->setAttribute('active', $status);
        $driver->setAttribute('available', $status);
        
        // Marquer que nous sommes dans le contexte online/offline pour éviter la vérification du wallet
        $driver->skip_wallet_check = true;
