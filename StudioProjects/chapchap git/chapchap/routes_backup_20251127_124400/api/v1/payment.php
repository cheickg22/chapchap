...
Route::prefix('payment')->namespace('Payment')->middleware(['auth:sanctum','throttle:30,1'])->group(function () {
     ...
     */
        ...
        // Orange Money WebPay - Nouvelle API
        Route::prefix('orange-money')->group(function () {
            Route::post('initiate-wallet-recharge', 'OrangeMoneyPaymentController@initiateWalletRecharge')->name('orange_money');
            Route::get('callback', 'OrangeMoneyPaymentController@callback');
            Route::get('cancel', 'OrangeMoneyPaymentController@cancel');
        });

        // Orange Money - Ancienne API (à conserver pour compatibilité)
 ...
 Route::prefix('payment')->namespace('Payment')->group(function () {
        ...
        // Orange Money Webhook (public route, called by Orange Money servers)
        Route::prefix('orange-money')->group(function () {
            Route::post('webhook', 'OrangeMoneyPaymentController@webhook');
        });
