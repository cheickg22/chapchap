...
use App\Http\Controllers\Web\Admin\MoovMoneyAdminController;
use App\Http\Controllers\Web\Admin\MoovMoneySettingsController;
/*
|--------------------------------------------------------------------------
| SPA Auth Routes
|--------------------------------------------------------------------------
...
Route::middleware('auth:sanctum', config('jetstream.auth_session'),)->group(function () {
    ...
    Route::namespace('Admin')->group(function () {
        Route::group(['prefix' => 'subscription', 'middleware' => 'permission:manage-subscription'], function(){
            ...
            Route::middleware('remove_empty_query')->get('/driver-plan-list/{driver}', 'SubscriptionController@driverSubscriptionList');
            Route::post('/driver-plan-expire/{driver}', 'SubscriptionController@expireSubsctiption');
        ...
        // Routes Moov Money Administration
        Route::controller(MoovMoneyAdminController::class)->group(function () {
            Route::group(['prefix' => 'moov-money-transactions'], function () {
                Route::get('/', 'index')->name('moov_money.index');
                Route::get('/list', 'list')->name('moov_money.list');
                Route::get('/export', 'export')->name('moov_money.export');
                Route::get('/statistics', 'statistics')->name('moov_money.statistics');
                Route::get('/{id}', 'show')->name('moov_money.show');
                Route::post('/{id}/update-status', 'updateStatus')->name('moov_money.update_status');
            });
        });

        // Routes Moov Money Settings
        Route::controller(MoovMoneySettingsController::class)->group(function () {
            Route::group(['prefix' => 'moov-money-settings'], function () {
                Route::get('/', 'index')->name('moov_money_settings.index');
                Route::post('/save', 'save')->name('moov_money_settings.save');
                Route::post('/test-connection', 'testConnection')->name('moov_money_settings.test');
                Route::get('/status', 'status')->name('moov_money_settings.status');
                Route::get('/logs', 'logs')->name('moov_money_settings.logs');
                Route::post('/force-sync', 'forceSync')->name('moov_money_settings.sync');
            });
        });
