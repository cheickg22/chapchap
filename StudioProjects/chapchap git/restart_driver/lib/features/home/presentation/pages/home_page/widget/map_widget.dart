...
import 'package:restart_user/features/home/presentation/pages/home_page/widget/bidding_ride/bidding_request_widget.dart';
import 'package:restart_user/features/home/presentation/pages/home_page/widget/bidding_ride/bidding_ride_list_widget.dart';
import 'package:restart_user/features/home/presentation/pages/home_page/widget/on_ride/on_ride_widget.dart';
import 'package:restart_user/features/home/presentation/pages/home_page/widget/accept_reject_widget.dart';
import 'package:restart_user/features/home/presentation/pages/home_page/widget/outstation_request_page.dart';
import 'package:restart_user/features/account/data/repository/acc_api.dart';
import 'package:restart_user/features/home/presentation/pages/home_page/widget/instand_ride/avatar_glow.dart';
        ...
        builder: (context, state) {
          ...
          return Stack(
            children: [
              ...
              if (userData != null && (userData!.metaRequest != null || state is ShowAcceptRejectState))
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        if (homeBloc.choosenRide != null &&
                                homeBloc.isOutstationRide && homeBloc.outStationList.isNotEmpty)
                            OutstationRequestWidget(bloc: homeBloc),
                        if (homeBloc.choosenRide != null &&
                                homeBloc.isNormalBidRide && homeBloc.rideList.isNotEmpty)
                            BiddingRequestWidget(bloc: homeBloc),
                      ],
                    ),
                  ),
                ),
              if (userData != null && (userData!.onTripRequest != null && userData!.metaRequest == null))
                Positioned(
                  bottom: 0,
                  child: SizedBox(
                    height: size.height,
        ...
        child: Column(
          ...
          children: [
            ...
            if (!homeBloc.showGetDropAddress &&
                ...
                userData!.enablePeakZoneFeature &&
                (userData!.metaRequest == null &&
                    userData!.onTripRequest == null) && homeBloc.showPeakZoneButton)
...
class _AcceptedRidesSheetState extends State<_AcceptedRidesSheet> {
  ...
  Future<List<Map<String, dynamic>>> _fetchOnTripRides() async {
    ...
    final List rides = (data is Map && data['data'] is List) ? data['data'] : [];
    final onGoingRides = rides.where((ride) {
      final isCompleted = ride['is_completed'] == 1 || ride['is_completed'] == true;
      final isCancelled = ride['is_cancelled'] == 1 || ride['is_cancelled'] == true;
      return !isCompleted && !isCancelled;
    }).toList();

    return onGoingRides.map<Map<String, dynamic>>((raw) {
      final user = (raw['userDetail'] != null && raw['userDetail']['data'] != null)
          ? raw['userDetail']['data']
          : {};
      return {
        'is_current': true,
        'request_id': raw['request_number']?.toString() ?? raw['id']?.toString() ?? '',
        ...
        'is_out_station': (raw['is_out_station'] == 1 || raw['is_out_station'] == true),
      };
    }).toList();
  }
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        if (snapshot.connectionState == ConnectionState.done)
                         MyText(
                            text: '${userData?.occupiedSeats} ${AppLocalizations.of(context)!.seatsTaken}',
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: MyText(
                               text: ' ${AppLocalizations.of(context)!.activeRidesMessage.replaceAll('2','${rides.length}')}',
                              textStyle: Theme.of(context).textTheme.bodySmall!.copyWith(
                                    color: AppColors.primary,
                                    fontWeight: FontWeight.w600,
                                  ),
                      ...
                      child: Column(
                        ...
                        children: rides
                            .map((ride) {
                              ...
                              final String id = ride['request_id']?.toString() ?? '';
                              final String reqNum = ride['request_number']?.toString() ?? '';
                              ...
                              return InkWell(
                                onTap: () {
                                  ...
                                  bloc.add(SelectOnTripRideEvent(requestId: id.isNotEmpty ? id : reqNum));
                                    Navigator.of(context).pop();
                                },
                                child: _buildRideCard(context, size, ride, true, isSelected),
                              );
                            })
                            .toList(),
                      ),
  ...
  Widget _buildRideCard(BuildContext context, Size size, Map<String, dynamic> ride, bool isCurrent, bool isSelected) {
