...
import 'package:restart_user/common/tobitmap.dart';
...
import 'package:restart_user/common/app_audios.dart';
import 'package:restart_user/common/app_constants.dart';
import 'package:restart_user/common/geohasher.dart';
import 'package:restart_user/core/utils/custom_payment_stream.dart';
import 'package:restart_user/core/utils/custom_text.dart';
import 'package:restart_user/db/app_database.dart';
import 'package:restart_user/features/home/application/usecase/ride_usecases.dart';
import 'package:restart_user/features/home/domain/models/address_auto_complete_model.dart';
import 'package:restart_user/features/home/domain/models/cancel_reason_model.dart';
import 'package:restart_user/features/home/domain/models/get_preferences_model.dart';
import 'package:restart_user/features/home/domain/models/goods_type_model.dart';
import 'package:url_launcher/url_launcher.dart';
import 'package:uuid/uuid.dart';
import 'package:restart_user/common/debouncer.dart';
import 'package:wakelock_plus/wakelock_plus.dart';
import 'package:restart_user/features/account/data/repository/acc_api.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HybridStatusService? _hybridStatusService; // Service hybride pour zones à faible connectivité
  ...
  HomeBloc() : super(HomeInitialState()) {
    ...
    on<NavigateMoovMoneyOnlyEvent>(navigateMoovMoneyOnly);
 ...
 FutureOr<void> toggleAcceptSharedRides(
  ...
  FutureOr<void> _selectOnTripRide(
    ...
    try {
      ...
      List dataList =
          (resp.data is Map && resp.data['data'] is List) ? resp.data['data'] : [];
      ...
      if (dataList.isNotEmpty) {
        ...
        final targetRide = dataList.firstWhere(
          (ride) {
            ...
            final matches = requestNumber == event.requestId || rideId == event.requestId;
            return matches;
          },
          orElse: () {
            return null;
        ...
        final raw = targetRide != null ? Map<String, dynamic>.from(targetRide) : Map<String, dynamic>.from(dataList.first);
        ...
        raw['requested_currency_symbol'] = raw['requested_currency_symbol'] ?? (raw['currency_symbol'] ?? '');
        raw['payment_type_string'] = raw['payment_type_string'] ?? raw['payment_type'] ?? '';
        ...
        // Route decision
        final arrived = (onTrip.arrivedAt != null && onTrip.arrivedAt!.isNotEmpty);
        ...
        } else if (onTrip.dropLat != null && onTrip.dropLng != null &&
        ...
        try {
          ...
          if (requestIdToSend != null) {
            await serviceLocator<HomeUsecase>().userDetails(requestId: requestIdToSend);
          } else {
      ...
      (success) async {
        ...
        if (userData != null && userData!.isDeletedAt == '') {
          if (userData!.serviceLocationId != null &&
              userData!.uploadedDocument == false) {
            errorMessage = 'Documents non validés\n\n'
                'Si vous avez déjà téléchargé tous vos documents :\n'
                '• Fermez et rouvrez l\'application\n'
                '• Vérifiez dans Profil > Documents\n'
                '• Contactez le support avec votre ID: ${userData!.id}\n\n'
                'Sinon, allez dans Profil > Documents pour télécharger les documents requis.';
          ...
          } else if (userData!.serviceLocationId != null &&
    ...
    data.fold(
      ...
      (success) {
        if (event.status == 1) {
          ...
          // Vérifier si c'est une requête Moov Money et mettre à jour le statut
          if (userData?.onTripRequest?.isMoovMoney == true) {
            debugPrint('🔔 Moov Money request accepted - updating status to accepted');
            FirebaseDatabase.instance.ref('requests').child(event.requestId).update({
              'moov_money_status': 'accepted',
              'status': 'accepted',
              'accepted_at': DateTime.now().toIso8601String(),
              'driver_id': userData!.id.toString(),
              'is_moov_money': true,
            });
            
            // Après quelques secondes, passer au statut "en_route"
            Future.delayed(Duration(seconds: 3), () {
              debugPrint('🔔 Moov Money driver en route');
              FirebaseDatabase.instance.ref('requests').child(event.requestId).update({
                'moov_money_status': 'en_route',
                'status': 'en_route',
              });
            });
          }
  ...
  FutureOr<void> rideArrived(
    ...
    // Pour les requêtes Moov Money, naviguer vers la page MoovMoney
    if (userData?.onTripRequest?.isMoovMoney == true) {
      debugPrint('🔔 Moov Money request - navigating to MoovMoneyRidePage');
  ...
  // Navigate to Moov Money page without calling arrived API
  FutureOr<void> navigateMoovMoneyOnly(
      NavigateMoovMoneyOnlyEvent event, Emitter<HomeState> emit) async {
    debugPrint('🔔 NavigateMoovMoneyOnly - Direct navigation to MoovMoneyRidePage');
    debugPrint('   Request ID: ${event.requestId}');
    
    // Émettre directement l'état de navigation sans appeler l'API
    emit(NavigateToMoovMoneyRidePageState(requestId: event.requestId));
  }
  ...
  FutureOr<void> rideStart(
    ...
    if (userData?.onTripRequest?.isMoovMoney == true) {
      debugPrint('🔔 Moov Money request detected - updating status to processing');
      
      // Mettre à jour Firebase pour indiquer le début du traitement
      ...
      // Pour Moov Money, pas besoin d'appeler l'API ride start
      isLoading = false;
      emit(LoadingStopState());
      emit(UpdateState());
      return;
...
//  Locations
  ...
  updateFirebaseData() async {
    print('🔥 updateFirebaseData called');
    if (userData != null) {
      print('📤 Updating Firebase for driver_${userData!.id}:');
      print('   - is_active: ${userData!.active == true ? 1 : 0}');
      print('   - is_available: ${userData!.available == true ? true : false}');
      print('   - onTripSearchNewRide: $onTripSearchNewRide');
      ...
      FirebaseDatabase.instance
        ...
        'shared_ride': (userData!.onTripRequest != null && userData!.onTripRequest!.sharedRide == true) ? 1 : 0,
      }).then((_) {
  ...
  Future<void> changeOnlineOffline(
    ...
    print('🚀 changeOnlineOffline called');
    print('📊 Current userData state:');
    print('   - active: ${userData?.active}');
    print('   - available: ${userData?.available}');
    print('   - serviceLocationId: ${userData?.serviceLocationId}');
    print('   - uploadedDocument: ${userData?.uploadedDocument}');
    print('   - approve: ${userData?.approve}');
    print('   - isDeletedAt: "${userData?.isDeletedAt}"');
    ...
    print('📡 Calling API onlineOffline...');
    ...
    data.fold(
      ...
      (success) {
        print('✅ API Success:');
        print('   - isOnline: ${success.isOnline}');
        print('   - success: ${success.success}');
        print('   - Setting userData.active to: ${success.isOnline}');
        print('   - Setting userData.available to: ${success.isOnline}');
        userData!.active = success.isOnline;
        userData!.available = success.isOnline;
        print('🔥 Calling updateFirebaseData()...');
        updateFirebaseData();
  ...
  FutureOr<void> onTripRequest(
    ...
    if (acceptSharedRides && rideList.length > 1) {
      ...
      if (rideList.isNotEmpty && rideList.first is Map) {
        ...
        currentRequestId = firstRide['id']?.toString() ?? firstRide['request_id']?.toString();
      }
  ...
  streamRequest() {
    ...
    requestStream = FirebaseDatabase.instance
        ...
        .equalTo(userData!.id.toString())  // ✅ Convertir en String pour correspondre à Firebase
        .onValue
        .listen((onData) {
            ...
            if (value['drivers'] != null &&
              ...
              if ((waitingData.isEmpty)) {
                ...
                if (value['is_out_station'] == true &&
                  ...
                  choosenRide = value['request_id'];  // Set choosenRide for outstation rides
                  outStationList.add(value);
                  add(UpdateEvent());
                }
              }
  ...
  streamRide() {
    ...
    // Démarrer le service hybride pour améliorer la fiabilité en zones à faible connectivité
    _startHybridStatusService();
  ...
  FutureOr<void> stopVerifyOtp(
    ...
    final data = await serviceLocator<RideUsecases>()
        .stopOtpVerify(stopId: event.stopId,requestId: event.requestId, otp: event.otp);
    data.fold(
      (error) {
  ...
  Future<void> loadPeakZones(
    ...
    zoneStreamRemove = FirebaseDatabase.instance
      ...
      polyline.removeWhere(
          (element) => element.polylineId.value.contains('peakzone_'));
      polyline.removeWhere((element) =>
          element.polylineId.value.contains('peakzone_route_'));
  ...
  Future<void> close() {
    ...
    _hybridStatusService?.dispose(); // Nettoyer le service hybride
    return super.close();
  }
...
void stopUserDetailsTimer() {
  ...
  /// Démarrer le service hybride pour le polling de statut
  /// Utilisé en complément de Firebase pour améliorer la fiabilité
  void _startHybridStatusService() {
    if (userData?.onTripRequest?.id == null) return;
    
    // Arrêter le service précédent s'il existe
    _hybridStatusService?.dispose();
    
    // Créer une nouvelle instance
    _hybridStatusService = HybridStatusService();
    
    // Déterminer le champ Firebase à écouter selon le type de requête
    String firebaseField = 'is_trip_start';
    if (userData!.onTripRequest!.isMoovMoney == true) {
      firebaseField = 'moov_money_status';
    }
    
    debugPrint('🔄 HomeBloc: Démarrage service hybride');
    debugPrint('   Request ID: ${userData!.onTripRequest!.id}');
    debugPrint('   Firebase field: $firebaseField');
    debugPrint('   Transport type: ${userData!.onTripRequest!.transportType}');
    
    // Démarrer l'écoute hybride
    _hybridStatusService!.startListening(
      requestId: userData!.onTripRequest!.id,
      onStatusChanged: (status) {
        debugPrint('📱 HomeBloc: Statut changé via service hybride: $status');
        
        // Recharger les détails de la requête pour avoir les données à jour
        if (!isClosed) {
          add(GetUserDetailsEvent());
        }
      },
      apiEndpoint: 'https://chapchap-livraison.com/api/v1/driver/request/{id}/status',
      firebaseStatusField: firebaseField,
    );
  }
}
