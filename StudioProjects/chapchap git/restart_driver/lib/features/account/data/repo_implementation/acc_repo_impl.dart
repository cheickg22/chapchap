...
import 'package:restart_user/features/account/domain/models/admin_chat_model.dart';
...
//get levels
  ...
  Future<Either<Failure, AddMyRouteAddressResponse>> addressUpdated({required double myRouteLat, required double myRouteLng, required String myRouteAddress}) async {
    ...
    try {
      Response response = await _accApi.addressUpdated(myRouteLat: myRouteLat, myRouteLng: myRouteLng, myRouteAddress: myRouteAddress);
      if (response.data == null || response.data == '') {
        return Left(GetDataFailure(message: 'User bad request'));
      } else if (response.data.toString().contains('error')) {
        return Left(GetDataFailure(message: response.data['error']));
        ...
        addMyRouteAddressResponse = AddMyRouteAddressResponse.fromJson(response.data);
      }
    } on FetchDataException catch (e) {
