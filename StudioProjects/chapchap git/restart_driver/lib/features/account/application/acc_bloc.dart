...
import 'package:restart_user/core/utils/custom_snack_bar.dart';
import 'package:restart_user/features/account/domain/models/driver_level_models.dart';
import 'package:restart_user/features/account/domain/models/driver_performance_model.dart';
import 'package:restart_user/features/account/domain/models/driver_rewards_model.dart';
import 'package:restart_user/features/account/domain/models/fleet_dashboard_model.dart';
import 'package:restart_user/features/account/domain/models/leader_board_model.dart';
import 'package:restart_user/features/account/domain/models/owner_dashboard_model.dart';
import 'package:restart_user/features/account/domain/models/report_section_model.dart';
import 'package:restart_user/features/account/domain/models/route_address_update_model.dart';
import 'package:restart_user/features/account/domain/models/subcription_list_model.dart';
import 'package:restart_user/features/account/domain/models/ticket_list_model.dart';
import 'package:restart_user/features/account/domain/models/ticket_names_model.dart';
import 'package:restart_user/features/account/domain/models/view_ticket_model.dart';
import 'package:restart_user/features/account/domain/models/withdraw_model.dart';
import 'package:restart_user/features/driverprofile/domain/models/service_location_model.dart';
...
import 'package:restart_user/features/account/application/usecase/acc_usecases.dart';
import 'package:restart_user/features/account/domain/models/contact_model.dart';
import 'package:restart_user/features/account/domain/models/driver_data_model.dart';
import 'package:restart_user/features/account/domain/models/earnings_model.dart';
import 'package:restart_user/features/account/domain/models/faq_model.dart';
import 'package:restart_user/features/account/domain/models/owner_vehicle_model.dart';
...
//Notification update
  ...
  void routeBookingInit(
      RouteBookingInitEvent event, Emitter<AccState> emit) {
    selectedMyRouteAddress = event.address;
    emit(UpdateState());
  }
  

  Future<void> _onMyRouteAddressUpdate(MyRouteAddressUpdateEvent event,Emitter<AccState> emit) async {
    isLoading = true;
    emit(UpdateState());
    ...
    result.fold(
      ...
      (response) {
        isLoading = false;
        emit(UpdateState());
        showToast(message: AppLocalizations.of(navigatorKey.currentState!.context)!.detailsUpdateSuccess);
        ...
        if (data.length > 1) {
          if (data[0].contains('+')) {
            ...
            if (val.length > 1 && !val[1].contains('+')) {
              if(event.isFromRoutePage == null) {selectedMyRouteAddress = '${val[1]} ${data.join(',')}';}
            ...
            if(event.isFromRoutePage == null) {selectedMyRouteAddress = data.join(',');}
            currentLocation = data.join(',');
          }
        } else {
          if(event.isFromRoutePage == null) {selectedMyRouteAddress = success.address;}
  ...
  Future<void> getCurrentLocation(
    ...
    if (status.isGranted || status.isLimited) {
      ...
      if(event.isFromRoutePage !=null && event.isFromRoutePage!){
        add(AccGeocodingLatLngEvent(lat: lat, lng: long,isFromRoutePage: event.isFromRoutePage));
      }
      if (googleMapController != null) {
        googleMapController!
            .animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: selectedMyRouteLatLng!,zoom:15)));
      }
      emit(UpdateState());
      if(event.isFromRoutePage !=null && event.isFromRoutePage!){
