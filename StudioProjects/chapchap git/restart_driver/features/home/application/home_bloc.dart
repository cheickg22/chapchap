...
//  Locations
  ...
  /// Helper method to safely add events
  void safeAdd(HomeEvent event) {
    if (!isClosed) {
      add(event);
    }
  }
    ...
    data.fold(
      ...
      (success) {
        ...
        if (success.isOnline == true) {
          ...
          // Solution: Ne pas appeler GetUserDetailsEvent après changement de statut
          // Le statut est déjà correct localement
        } else {
          showPeakZones = false;
          polygons.removeWhere(
              (element) => element.polygonId.value.contains('peakzone_'));
  ...
  Future<void> biddingFareIncreaseDecrease(
    ...
    final double currentPrice = bidRideAmount.text.isNotEmpty
        ? double.parse(bidRideAmount.text)
        : double.parse(acceptedRideFare);
    final double step = double.parse(userData!.biddingAmountIncreaseOrDecrease);

    final double maxAllowedPrice = double.parse(acceptedRideFare) +
        ((double.parse(userData!.biddingHighPercentage) / 100) *
            double.parse(acceptedRideFare));
    final double minAllowedPrice = userData!.biddingLowPercentage == "0"
        ? double.parse(acceptedRideFare)
        : double.parse(acceptedRideFare) -
    ...
    if (event.isIncrease) {
      ...
      if (nextPrice <= maxAllowedPrice) {
        bidRideAmount.text = nextPrice.toStringAsFixed(2);
        isBiddingIncreaseLimitReach = false;
      } else {
        bidRideAmount.text = maxAllowedPrice.toStringAsFixed(2);
      ...
      if (nextPrice >= minAllowedPrice) {
        bidRideAmount.text = nextPrice.toStringAsFixed(2);
        isBiddingDecreaseLimitReach = nextPrice == minAllowedPrice;
      } else {
        bidRideAmount.text = minAllowedPrice.toStringAsFixed(2);
  ...
  Future<void> loadPeakZones(
    ...
    zoneStreamRemove =
        FirebaseDatabase.instance.ref("peak-zones").onChildRemoved.listen(
      (e) {
        showDemandArea = false;
        polygons.removeWhere((element) =>
            element.polygonId.value.contains('peakzone_${e.snapshot.key}'));
        polyline.removeWhere((element) =>
            element.polylineId.value.contains('peakzone_${e.snapshot.key}'));
        polyline.removeWhere((element) => element.polylineId.value
            .contains('peakzone_route_${e.snapshot.key}'));
...
//SELECTED PREFERENCES
  ...
  Future<void> clearTempPreferenceEvent(
    ...
    tempSelectPreference
      ..clear()
      ..addAll(selectedPreferenceDetailsList);
    add(UpdateEvent());
  }
}
