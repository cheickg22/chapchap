...
import 'package:restart_user/common/app_arguments.dart';
import 'package:restart_user/common/app_constants.dart';
import 'package:restart_user/common/app_images.dart';
  ...
  FutureOr<void> getAddressFromLatLng(
    ...
    if (event.lat != 0.0 && event.lng != 0.0) {
      ...
      data.fold((error) {
        ...
        final parts = success.address.toString().split(',');
        if (parts.length > 1) {
          if (parts[0].contains('+')) {
            ...
            if (val.length > 1 && !val[1].contains('+')) {
              if (event.isFromRoutePage == null) {
                selectedMyRouteAddress = '${val[1]} ${parts.join(',')}';
              }
              currentLocation = '${val[1]} ${parts.join(',')}';
            } else {
              if (event.isFromRoutePage == null) {
                selectedMyRouteAddress = parts.join(',');
              }
    ...
    if (mapType == 'google_map') {
      ...
      if (event.address.isEmpty) {
        ...
        data.fold(
          ...
          (success) {
            selectedMyRouteAddress = success.address;
            emit(UpdateState()); // ✅ trigger UI update here
          },
        );
      } else {
        selectedMyRouteAddress = event.address;
        emit(UpdateState()); // ✅ also trigger UI when address is given
      }

      fmController.move(
        fmlt.LatLng(event.position!.latitude, event.position!.longitude),
        13,
      );
